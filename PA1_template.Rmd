...
The mean is `r MeanSteps` and the median is `r MedianSteps` and both nearly overlap each other. 
...
```
The time interval that contains the max number of steps is `r maxInterval`
...
```
The new mean is `r MeanNAFilled` and the new median is `r MedianNAFilled`.
...
```
The new mean is `r MeanNAFilled` and the new median is `r MedianNAFilled`. They are the same and they overlap on the graph
